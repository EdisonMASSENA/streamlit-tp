...
class AllocineSpider(scrapy.Spider):
    ...
    def parse_allocine(self, response):
        ...
        for film in list_film:
            ...
            # Durée du film
            try:
                item['time'] = film.css('div.meta-body-item.meta-body-info::text').get()
            except:
                item['time'] = 'None'

            # Genre cinématographique
            try:
                item['genre'] = film.css('div.meta-body-item.meta-body-info span::text').getall()
            except:
                 item['genre'] = 'None'
            ...
            # Description du film
            try:
                item['desc'] = film.css('div.content-txt::text').get()
            except:
                item['desc'] = 'None'
